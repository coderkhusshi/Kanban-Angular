...
export class NavbarComponent implements OnInit {
 ...
 tasks : Task[] | any;
 taskid : any;
  constructor(private localstorage: StorageService, private fb: FormBuilder, private service: TaskService, private router :Router) { 
    this.taskForm = this.fb.group({
      title: ['', {
        validators: [Validators.required],
        updateOn: 'blur',
  ...
  addTask():void {
      let tas: any = this.taskForm.value;
     alert(JSON.stringify(tas));
       this.service.addTask(tas)
         .subscribe(data => {
           alert("Task  is created");
           this.service.showTask().subscribe(custs => {
             this.tasks = custs;
           });
         },
         );
    ...
    if(formData.title && formData.description && formData.status > -1) {
      ...
      //    this.service.addTask(tas)
      //      .subscribe(data => {
      //        alert("Task with Id " + data + " is created");
      //        this.service.showTask().subscribe(custs => {
      //          this.tasks = custs;
      //        });
      //      },
